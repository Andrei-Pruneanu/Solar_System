...
;; Solar System Simulator - Milestone Submission (Fixed Gallery Sizes & Direct Loading)
;; Features: Images, Zoom, Orbit Lines, Speed Control, Camera Lock, Info Panel, Satellites, Asteroids, Rare Comets, Photo Gallery
...
;; --- GALLERY IMAGES (PRE-LOADED TO FIX LAG) ---
...
;; IF PROGRAM CRASHES HERE: You are missing a file in the /img/ folder!
...
;; NEW CONSTANTS FOR FIXED GALLERY SIZE
...
(define SPEED-LEVELS
  (list (list "Paused" 0.0) (list "30 Min/Sec" 0.0015) (list "5 Hours/Sec" 0.015) (list "1 Day/Sec" 0.07)
        (list "15 Days/Sec" 1.0) (list "1 Month/Sec" 2.0) (list "6 Months/Sec" 12.0) (list "1 Year/Sec" 24.0)))
...
;; Data Type: Location
;; Structure: (make-location Number Number)
;; Interpretation: Cartesian coordinates (x, y) relative to a center point.
...
;; Data Type: Satellite (Moon)
;; Structure: (make-satellite String Number Number String Number Number)
;; Interpretation: 
...
;; Template:
#;
(define (fn-for-satellite s)
  (... (satellite-name s) ... (satellite-radius s) ... (satellite-dist s) ...
       (satellite-color s) ... (satellite-angle s) ... (satellite-speed s) ...))


;; Data Type: CelestialBody (Planet)
;; Structure: (make-celestial-body String Number Number String Number Number List<Satellite>)
;; Interpretation:
;; - name: name of the planet
;; - radius: visual radius
;; - orbit-au: distance from the sun in pixels
;; - color: visual color (fallback)
;; - angle: current orbital angle
;; - angular-speed: orbital speed
...
(define (fn-for-body b)
  (... (celestial-body-name b) ... (celestial-body-radius b) ... (celestial-body-orbit-au b) ...
       (celestial-body-color b) ... (celestial-body-angle b) ... (celestial-body-angular-speed b) ...
       (fn-for-lob (celestial-body-satellites b)) ...))


;; Data Type: Asteroid
;; Structure: (make-asteroid Number Number Number Number String)
;; Interpretation: represents a rock in the asteroid belt.
;; - angle: current angle
...
;; Template:
#;
(define (fn-for-asteroid a)
  (... (asteroid-angle a) ... (asteroid-dist a) ... (asteroid-speed a) ...
       (asteroid-size a) ... (asteroid-color a) ...))


;; Data Type: Comet
;; Structure: (make-comet Number Number Number Number Number)
...
(define (fn-for-comet c)
  (... (comet-x c) ... (comet-y c) ... (comet-vx c) ... (comet-vy c) ... (comet-size c) ...))


;; Data Type: SimState
;; Structure: (make-sim-state List<CelestialBody> Number Number Number String Number String Number List<Asteroid> List<Comet>)
;; Interpretation: The complete state of the world.
...
;; Template:
#;
(define (fn-for-sim-state s)
  (... (sim-state-bodies s) ... (sim-state-zoom s) ... (sim-state-mouse-x s) ...
       (sim-state-mouse-y s) ... (sim-state-locked-body-name s) ... (sim-state-speed-index s) ...
       (sim-state-active-tab s) ... (sim-state-gallery-index s) ...
       (sim-state-asteroid-belt s) ... (sim-state-active-comets s) ...))
...
; This is instant because it doesn't read from disk anymore.
; header:
...
(define (generate-stars n scene)
  (cond [(= n 0) scene] 
        [else (place-image (circle (max 1 (random 3)) "solid" "white") (random SCENE-WIDTH) (random SCENE-HEIGHT) (generate-stars (- n 1) scene))]))            

(define BLACK-BACKGROUND (empty-scene SCENE-WIDTH SCENE-HEIGHT "black"))
(define EMPTY-SCENE (generate-stars 200 BLACK-BACKGROUND))
...
(define (next-satellite-state moon speed-mult)
  (make-satellite (satellite-name moon) (satellite-radius moon) (satellite-dist moon) (satellite-color moon)
                  (+ (satellite-angle moon) (* (satellite-speed moon) speed-mult)) (satellite-speed moon)))
...
(define (next-body-state body speed-mult)
  (make-celestial-body (celestial-body-name body) (celestial-body-radius body) (celestial-body-orbit-au body) (celestial-body-color body)
   (+ (celestial-body-angle body) (* (celestial-body-angular-speed body) speed-mult)) (celestial-body-angular-speed body)
...
(define (next-asteroid ast speed-mult)
  (make-asteroid (+ (asteroid-angle ast) (* (asteroid-speed ast) speed-mult)) 
                 (asteroid-dist ast) (asteroid-speed ast) (asteroid-size ast) (asteroid-color ast)))
...
(define (update-asteroids asteroids speed-mult)
  (cond [(empty? asteroids) '()] 
        [else (cons (next-asteroid (first asteroids) speed-mult) (update-asteroids (rest asteroids) speed-mult))]))
...
(define (next-comet c)
  (make-comet (+ (comet-x c) (comet-vx c)) (+ (comet-y c) (comet-vy c)) (comet-vx c) (comet-vy c) (comet-size c)))
...
(define (maybe-spawn-comet current-comets)
  (if (< (random 3000) 2) 
      (cons (make-comet -100 (random SCENE-HEIGHT) (+ 2 (random 5)) (- (random 3) 1.5) (+ 2 (random 3))) current-comets)
      current-comets))
...
(define (next-world current-state)
  (local [
          ...
          (define (update-bodies bodies)
            (cond [(empty? bodies) '()] 
                  [else (cons (next-body-state (first bodies) speed-mult) (update-bodies (rest bodies)))]))]             
    (make-sim-state 
...
(define (draw-satellites-for-planet moons planet-x planet-y zoom scene)
  (cond [(empty? moons) scene]
        [else (local [(define moon (first moons))
...
(define (draw-asteroids asteroids target-loc zoom scene)
  (cond [(empty? asteroids) scene]
        [else (local [(define ast (first asteroids))
  ...
  (local [
          ...
          (define base-panel 
            (overlay/align "middle" "top"
                           (above buttons-row
                                  (rectangle 1 15 "solid" "transparent") (text (string-upcase locked-name) 22 "yellow") (rectangle 1 15 "solid" "transparent") content-text)
                           info-panel-bg))
          
          (define hud (above/align "left" (text (string-append "Camera: " locked-name) 16 "white") (text "" 2 "black") (text (string-append "Speed: " speed-label) 16 "white")))
          (define legend (above/align "left" (text "LEGEND" 16 "yellow") (text "Arrows: Zoom" 14 "white") (text "I / D: Speed" 14 "white") (text "Click: Lock" 14 "white")))]
